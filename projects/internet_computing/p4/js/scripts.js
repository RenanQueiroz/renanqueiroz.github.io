...
$(function () {
    ...
    startNewGame = function () {
        ...
        turn = "X";
        document.getElementById("playerOscore").classList.add("text-muted");
        document.getElementById("playerXscore").classList.remove("text-muted");
        document.getElementById("gameResult").innerHTML = EMPTY;
        score = {"X": 0, "O": 0};
        moves = 0;
        squares.forEach(function (square) {square.html(EMPTY);});
        squares.forEach(function (square) {square.css("font-size", "1.5em");});
        isGameOver = false;
    },
    ...
    set = function () {
        ...
        if (win(score[turn])) {
            ...
            document.getElementById("xpts").innerHTML = xPts;
            document.getElementById("opts").innerHTML = oPts;
            document.getElementById("gameResult").innerHTML = turn + " Wins!";
            document.getElementById("restartBtn").style.visibility = 'visible';
        } else if (moves === SIZE * SIZE) {
            isGameOver = true;
            document.getElementById("gameResult").innerHTML = "Cat's game!";
            document.getElementById("restartBtn").style.visibility = 'visible';
        } else {
            turn = turn === "X" ? "O" : "X";
            if(turn === "X"){
                ...
                document.getElementById("playerXscore").classList.add("text-muted");
                document.getElementById("playerOscore").classList.remove("text-muted");
            }
        }
    },
